...
listarProposicoes <- function(sigla = "",
  ...
  parsedOutput <- xmlParse(GET('http://www.camara.gov.br/SitCamaraWS/Proposicoes.asmx/ListarProposicoes?',
                               query = list(Sigla = sigla,
                                            ...
                                            codEstado = "",
                                            codOrgaoEstado = "")))
  proposicao <- xmlToDataFrame(getNodeSet( parsedOutput, "//proposicao"), stringsAsFactors = F)[,c(1,2,4,5,7,8,9,13,16,17)]
  ...
  names(proposicao) <- c("id","nome","numero","ano","datApresentacao","txtEmenta",
                         "txtExplicacaoEmenta","qtdAutores","indGenero",
                         "qtdOrgaosComEstado","tipoProposicao.id","tipoProposicao.sigla",
                         "tipoProposicao.nome","orgaoNumerador.id","orgaoNumerador.sigla",
                         "orgaoNumerador.nome","codRegime","txtRegime","apreciacao.id",
                         "txtApreciacao","txtNomeAutor","idecadastro","codPartido",
                         "txtSiglaPartido","txtSiglaUF","datDespacho","txtDespacho",
                         "situacao.id","situacao.descricao","codOrgaoEstado",
                         "siglaOrgaoEstado","codProposicaoPrincipal","proposicaoPrincipal")
  return(proposicao)
}
